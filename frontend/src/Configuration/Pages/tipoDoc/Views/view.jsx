...
export default function View() {
  ...
  const departamentos = useFetch(RoutesURL.TIPO_DOCUMENTOS);
    ...
    (id) => () => {
      messageAlert().then(async (result) => {
        if (result.isConfirmed) {
          axiosToken
            .delete(`${RoutesURL.TIPO_DOCUMENTOS}/${id}`)
            .then(() => {
  ...
  const columns = useMemo(
    () => [
      ...
      { field: "nombre", headerName: "Departamentos / Unidades", width: 300 },
    ],
    [deleteRow]
  );
  ...
  return departamentos.loading ? (
    <>
      <Titles title="Tipo de Documentos" />
      <Box m={2}>
        <Box m={1}>
          <Button
            size="small"
