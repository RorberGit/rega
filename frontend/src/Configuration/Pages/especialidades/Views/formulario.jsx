...
export default function Formulario() {
    ...
    if (action === "update") {
      axiosToken
        ...
        .catch((error) => {
          ...
          setError("A ocurrido un error cargando la especialidad");
        })
  ...
  const handleFormSubmit = async (values) => {
    if (action === "create") {
      await axiosToken
        ...
        .then(() => {
          router.push(`/${RoutesURL.ROOT}/${RoutesURL.ESPECIALIDADES}`);
          Message(
            `Especialidad ${values.nombre} creada exitosamente`,
            "success"
          );
        })
        .catch((error) => {
      ...
      await axiosToken
        ...
        .then(() => {
          router.push(`/${RoutesURL.ROOT}/${RoutesURL.ESPECIALIDADES}`);
          Message(
            `Especialidad ${values.nombre} actualizada exitosamente`,
            "success"
          );
        })
  ...
  return loading ? (
    <>
      <Titles
        title={
          action === "create" ? "Crear Especialidad" : "Actualizar Especialidad"
        }
      />
      {error && (
